...
:func:`src.agents.checkpointer.async_provider.make_checkpointer`.
...
from src.config.stream_bridge_config import get_stream_bridge_config
...
async def make_stream_bridge(config=None) -> AsyncIterator[StreamBridge]:
    ...
    if config is None or config.type == "memory":
        from src.runtime.stream_bridge.memory import MemoryStreamBridge
