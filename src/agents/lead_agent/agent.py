...
def make_lead_agent(config: RunnableConfig):
    ...
    tool_groups_override = cfg.get("tool_groups", None)
    ...
    logger.info(
        "Create Agent(%s) -> thinking_enabled: %s, reasoning_effort: %s, model_name: %s, is_plan_mode: %s, subagent_enabled: %s, max_concurrent_subagents: %s, include_mcp: %s, tool_groups_override: %s",
    ...
    if is_bootstrap:
        ...
        return create_agent(
            ...
            tools=get_available_tools(model_name=model_name, subagent_enabled=subagent_enabled, include_mcp=include_mcp, groups=tool_groups_override) + [setup_agent],
            middleware=_build_middlewares(config, model_name=model_name),
            system_prompt=apply_prompt_template(subagent_enabled=subagent_enabled, max_concurrent_subagents=max_concurrent_subagents, available_skills=set(["bootstrap"])),
            state_schema=ThreadState,
        )

    effective_groups = tool_groups_override if tool_groups_override is not None else (agent_config.tool_groups if agent_config else None)

    # Default lead agent (unchanged behavior)
    return create_agent(
        model=create_chat_model(name=model_name, thinking_enabled=thinking_enabled, reasoning_effort=reasoning_effort),
        tools=get_available_tools(model_name=model_name, groups=effective_groups, include_mcp=include_mcp, subagent_enabled=subagent_enabled),
