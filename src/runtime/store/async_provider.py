...
from src.runtime.store.provider import POSTGRES_CONN_REQUIRED, POSTGRES_STORE_INSTALL, SQLITE_STORE_INSTALL, ensure_sqlite_parent_dir, resolve_sqlite_conn_str
...
async def _async_store(config) -> AsyncIterator[BaseStore]:
    ...
    The ``config`` argument is a :class:`src.config.checkpointer_config.CheckpointerConfig`
...
async def make_store() -> AsyncIterator[BaseStore]:
    ...
    :func:`src.agents.checkpointer.async_provider.make_checkpointer` so
