...
from src.tools.builtins.tool_search import reset_deferred_registry
...
def _is_host_bash_tool(tool: object) -> bool:
    ...
    if group == "bash":
        return True
    if use == "src.sandbox.tools:bash_tool":
        return True
    return False
...
def get_available_tools(
    ...
    if getattr(skill_evolution_config, "enabled", False):
        from src.tools.skill_manage_tool import skill_manage_tool
    ...
    if include_mcp:
        try:
            ...
            effective_extensions_config = extensions_config or ExtensionsConfig.from_file()
            if effective_extensions_config.get_enabled_mcp_servers():
                ...
                if mcp_tools:
                    ...
                    if config.tool_search.enabled:
                        from src.tools.builtins.tool_search import DeferredToolRegistry, set_deferred_registry
                        from src.tools.builtins.tool_search import tool_search as tool_search_tool
    ...
    try:
        ...
        from src.tools.builtins.invoke_acp_agent_tool import build_invoke_acp_agent_tool
